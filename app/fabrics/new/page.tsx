...
import { MagnifyingGlassIcon, PhotoIcon, PlusIcon, SparklesIcon } from '@heroicons/react/24/outline'
...
export default function FabricsNewPage() {
  ...
  return (
    <div className="min-h-screen bg-macos-bg-secondary">
      <PageHeader
        title="Vải Mới"
        subtitle={`${filteredFabrics.length} loại vải mới (30 ngày gần đây)`}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl p-12 text-center border border-macos-border-light">
            ...
            <p className="text-sm text-macos-text-secondary mb-6">
              {searchTerm ? 'Không tìm thấy vải phù hợp với từ khóa tìm kiếm' : 'Thêm vải đầu tiên vào danh mục Vải Mới'}
            </p>
            <button
              onClick={() => setUploadModalOpen(true)}
